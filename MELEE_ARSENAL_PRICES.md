# Melee Arsenal Crate — Список предметов и цены

Ящик `CrateSyndicateMeleeArsenal`. Бюджет: **40 TC**. Контекст: **арена, дуэль 1 на 1**.

Тот же пул, что и Full Arsenal, но **без любого дальнобойного оружия** — только ближний бой, броня, щиты, утилита, гранаты, медицина и поддержка.

Лимиты категорий за одно открытие:
- Броня (`MeleeArsenalArmor`): **макс. 2**
- Шлемы/противогазы (`MeleeArsenalHead`): **макс. 1**
- Гранаты (`MeleeArsenalGrenade`): **макс. 3**

**Гарантия:** в каждом ящике всегда есть **минимум одно оружие ближнего боя** (категория `MeleeArsenalWeapon` — секции «Ближний бой», «Ближний бой (особое)», «Ближний бой (дополнительно)»). Оно выдаётся первым в рамках бюджета, остальное добивается случайно.

Цены калиброваны по аплинку: предметы из аплинка = цена аплинка; предметы из бандлей ≈ цена бандля минус ~2 TC за упаковку.

Связанные файлы:
- Пул: `Resources/Prototypes/_Wega/Catalog/melee_arsenal_pool.yml`
- Локаль: `Resources/Locale/ru-RU/_wega/store/uplink-catalog.ftl`
- Полный арсенал (с дальнобоем): `FULL_ARSENAL_PRICES.md`
...
| Джаггернаут | `ClothingOuterHardsuitJuggernaut` | 12 TC |
| Отряд смерти | `ClothingOuterHardsuitDeathsquad` | 35 TC |
...
| Синдикат Командир | `ClothingOuterHardsuitSyndieCommander` | 16 TC |
...
## Бандли (ближний бой и поддержка)
...
| Портативное зарядное устройство | `PortableRecharger` | 2 TC |
...
| Экстренный медпен | `EmergencyMedipen` | 2 TC |
| Антитоксиновый медпен | `AntiPoisonMedipen` | 2 TC |
| Боевой медпен | `CombatMedipen` | 4 TC |
| Медпен выживания | `SurvivalMedipen` | 2 TC |
| Автоинъектор от ушибов | `BruteAutoInjector` | 1 TC |
| Автоинъектор от ожогов | `BurnAutoInjector` | 1 TC |
| Антирадиационный автоинъектор | `RadAutoInjector` | 1 TC |
| Автоинъектор от удушья | `AirlossAutoInjector` | 1 TC |
| Автоинъектор от колотых ран | `PunctAutoInjector` | 1 TC |
| Автоинъектор «Пира» | `PyraAutoInjector` | 1 TC |
| Коробка автоинъекторов (полная) | `BoxAutoInjectorAll` | 3 TC |
| Бутылочка бикаридина | `ChemistryBottleBicaridine` | 1 TC |
| Бутылочка дермалина | `ChemistryBottleDermaline` | 1 TC |
| Бутылочка омнизина | `ChemistryBottleOmnizine` | 2 TC |
| Бутылочка трикордразина | `ChemistryBottleTricordrazine` | 1 TC |
| Бутылочка келотана | `ChemistryBottleKelotane` | 1 TC |
| Бутылочка диловена | `ChemistryBottleDylovene` | 1 TC |
| Бутылочка инапровалина | `ChemistryBottleInaprovaline` | 1 TC |
| Бутылочка эпинефрина | `ChemistryBottleEpinephrine` | 1 TC |
| Бутылочка транексамовой кислоты | `ChemistryBottleTranexamicAcid` | 1 TC |
| Бутылочка аритразина | `ChemistryBottleArithrazine` | 1 TC |
...
| Предмет | Entity | Цена |
|---|---|---|
| Рюкзак Синдиката | `ClothingBackpackSyndicate` | 3 TC |
| Вещмешок Синдиката | `ClothingBackpackDuffelSyndicate` | 3 TC |
...
| Предмет | Entity | Цена |
|---|---|---|
| Комбинезон оперативника | `ClothingUniformJumpsuitOperative` | 3 TC |
| Чёрный комбинезон | `ClothingUniformJumpsuitColorBlack` | 2 TC |
| Комбинезон отряда смерти | `ClothingUniformJumpsuitDeathSquad` | 3 TC |
| Комбинезон Синего щита | `ClothingUniformJumpsuitBlueShield` | 2 TC |
...
| Магботы Синдиката | `ClothingShoesBootsMagSyndie` | 2 TC |
| Боевые магботы | `ClothingShoesBootsMagCombat` | 3 TC |
| Улучшенные магботы Синдиката | `ClothingShoesBootsMagAdvSynd` | 8 TC |
| Улучшенные магнитные ботинки | `ClothingShoesBootsMagAdv` | 5 TC |
| Слепящие магботы | `ClothingShoesBootsMagBlinding` | 8 TC |
...
| Трость-клинок | `CaneSheathFilled` | 3 TC |
...
| Предмет | Entity | Цена |
|---|---|---|
| Облегчённая базовая броня | `ClothingOuterArmorBasicSlim` | 3 TC |
| Панцирь капитана | `ClothingOuterArmorCaptainCarapace` | 5 TC |
| Панцирь ЦентКома | `ClothingOuterArmorCentcommCarapace` | 6 TC |
| Жилет детектива | `ClothingOuterVestDetective` | 3 TC |
| Броня тамплиера | `ClothingOuterArmorTemplar` | 5 TC |
| Броня Синего щита | `ClothingOuterArmorBlueShield` | 5 TC |
| Костюм исследователя | `ClothingOuterArmorExplorerSuit` | 6 TC |
| Костюм исследователя (хаки) | `ClothingOuterArmorExplorerSuitKhaki` | 6 TC |
| Костюм исследователя (медицинский) | `ClothingOuterArmorExplorerSuitMedical` | 7 TC |
| Панцирь шерифа | `ClothingOuterArmorSherifCarapace` | 5 TC |
| Белый панцирь капитана | `ClothingOuterArmorCaptainCarapaceWhite` | 5 TC |

## Скафандры (дополнительно)

| Предмет | Entity | Цена |
|---|---|---|
| Скафандр спатио | `ClothingOuterHardsuitSpatio` | 7 TC |
| Утилизаторский скафандр | `ClothingOuterHardsuitSalvage` | 8 TC |
| Скафандр «Голиаф» | `ClothingOuterHardsuitGoliath` | 8 TC |
| Пустотный скафандр | `ClothingOuterHardsuitVoid` | 10 TC |
| Улучшенный пустотный скафандр | `ClothingOuterHardsuitVoidAdvanced` | 12 TC |
...
| Шрапнельная граната «Стингер» | `GrenadeStinger` | 2 TC |
...
## Боевые перчатки и кастеты

| Предмет | Entity | Цена |
|---|---|---|
| Боевые перчатки | `ClothingHandsGlovesCombat` | 2 TC |
| Боевые перчатки наёмника | `ClothingHandsMercGlovesCombat` | 2 TC |
| Беспалые перчатки наёмника | `ClothingHandsGlovesMercFingerless` | 1 TC |
| Боевые перчатки Синего щита | `ClothingHandsGlovesCombatBlueShield` | 3 TC |
| Перчатки «Полярная звезда» | `ClothingHandsGlovesNorthStar` | 8 TC |
| Перчатки крав-мага | `ClothingHandsGlovesKravMaga` | 5 TC |
| Контузящие перчатки | `ClothingHandsGlovesConcussiveGauntlets` | 5 TC |
| Силовая перчатка | `ClothingHandsGlovesPowerglove` | 3 TC |
| Перчатки космического ниндзя | `ClothingHandsGlovesSpaceNinja` | 4 TC |
| Боксёрские перчатки (красные, утяжелённые) | `ClothingHandsGlovesBoxingRiggedRed` | 4 TC |
| Боксёрские перчатки (синие, утяжелённые) | `ClothingHandsGlovesBoxingRiggedBlue` | 4 TC |
| Боксёрские перчатки (зелёные, утяжелённые) | `ClothingHandsGlovesBoxingRiggedGreen` | 4 TC |
| Боксёрские перчатки (жёлтые, утяжелённые) | `ClothingHandsGlovesBoxingRiggedYellow` | 4 TC |
...
| Костоломные кастеты | `ClothingHandsKnuckleBoneCrushers` | 3 TC |
